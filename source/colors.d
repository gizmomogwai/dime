...
string asMixin(T)() {
  ...
  foreach (immutable ansiColor; [EnumMembers!T]) {
    ...
    res ~= "auto bg%3$s(string s) { return StringWithBackground!%2$s(s, %2$s.%1$s); }\n".format(ansiColor, typeof(T.init).stringof, name);
    res ~= "auto bg%3$s(StringWithForeground!%2$s s) { return StringWithBoth!%2$s(s.s, s.fg, %2$s.%1$s); }\n".format(ansiColor, typeof(T.init).stringof, name);
  }
  return res;
}
...
auto bgR(string s) { return StringWithBackground!TTT(s, TTT.r); }
auto bgR(StringWithForeground!TTT s) { return StringWithBoth!TTT(s.s, s.fg, TTT.r); }
`);
}
